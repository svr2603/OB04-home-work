...
# Программа должна быть спроектирована таким образом, чтобы легко можно было добавлять
...
# - Создайте несколько классов, унаследованных от `Weapon`,
# например, `Sword` и `Bow`. Каждый из этих классов реализует метод `attack()`
...
class Fighter:
    ...
    def fight(self):
        if self.weapon:
            return f"{self.name} {self.weapon.attack()}. Монстр побежден!"
        else:
            return f"{self.name} не вооружен. Монстр побежден!"
